...
describe("advanced test", () => {
  ...
  describe("createShoppingCart >", () => {
    ...
    it("상품의 수량을 업데이트할 수 있다.", () => {
      const product = { id: "p1", name: "상품1", price: 10000 };
      cart.addItem({ product });
      cart.updateQuantity({ id: "p1", quantity: 3 });
    ...
    it("수량이 0으로 업데이트될 때 해당 상품이 제거된다.", () => {
      const product = { id: "p1", name: "상품1", price: 10000 };
      cart.addItem({ product });
      cart.updateQuantity({ id: "p1", quantity: 0 });
      expect(cart.getItems()).toHaveLength(0);
    });
    ...
    it("총액을 정확하게 계산해야 한다", () => {
      ...
      cart.addItem({ product: product2, quantity: 2 });
    ...
    describe("할인 로직", () => {
      ...
      describe("상품별 할인 적용", () => {
        it.each([
          ...
          ({ product, expected }) => {
            cart.addItem({ product, quantity: 10 });
      ...
      it("30개 이상 구매 시 전체 25% 할인이 적용되어야 한다", () => {
        cart.addItem({ product: products[0], quantity: 15 });
        cart.addItem({ product: products[1], quantity: 15 });
        expect(cart.getTotal().total).toBe(337500); // (10000 * 15 + 20000 * 15) * 0.75
      });

      it("개별 상품 할인과 전체 할인 중 더 큰 할인이 적용되어야 한다", () => {
        cart.addItem({ product: products[0], quantity: 15 }); // 10% 할인
        cart.addItem({ product: products[2], quantity: 15 }); // 20% 할인
